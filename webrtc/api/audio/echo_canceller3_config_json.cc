...
namespace webrtc {
namespace {
void ReadParam(const SJson::Value& root, std::string param_name, bool* param) {
  RTC_DCHECK(param);
  bool v;
...
void ReadParam(const SJson::Value& root, std::string param_name, size_t* param) {
  RTC_DCHECK(param);
  int v;
...
void ReadParam(const SJson::Value& root, std::string param_name, int* param) {
  RTC_DCHECK(param);
  int v;
...
void ReadParam(const SJson::Value& root, std::string param_name, float* param) {
  RTC_DCHECK(param);
  double v;
...
void ReadParam(const SJson::Value& root,
               std::string param_name,
               EchoCanceller3Config::Filter::RefinedConfiguration* param) {
  RTC_DCHECK(param);
  SJson::Value json_array;
...
void Aec3ConfigFromJsonString(absl::string_view json_string,
  ...
  *parsing_successful = true;
  ...
  SJson::Value root;
  bool success = SJson::Reader().parse(std::string(json_string), root);
  if (!success) {
